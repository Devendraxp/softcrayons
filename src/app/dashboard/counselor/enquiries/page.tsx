...
import {
  CounselorEnquiryList,
  CounselorEnquiryListHeader,
  CounselorViewDetailsDialog,
  CounselorEditNotesDialog,
  type Enquiry,
  type EnquiryStatus,
} from "@/components/dashboard/counselor-enquiries";
import { Search, RefreshCw } from "lucide-react";
import { toast } from "sonner";

// No ASSIGNED tab - counselor only sees their assigned enquiries
// NEW tab shows all enquiries assigned to this counselor (regardless of status)
...
export default function CounselorEnquiriesPage() {
  ...
  const fetchCounts = React.useCallback(async () => {
    try {
      const response = await fetch("/api/counselor/enquiries?counts=true");
  ...
  const fetchEnquiries = React.useCallback(async () => {
    ...
    try {
      ...
      // Filter by status (no subStatus needed since all are assigned to this counselor)
      if (activeTab !== "ALL") {
        params.set("status", activeTab);
      }

      const response = await fetch(`/api/counselor/enquiries?${params.toString()}`);
      const data = await response.json();
  ...
  const handleStatusChange = async (enquiry: Enquiry, status: EnquiryStatus) => {
    try {
      const response = await fetch(`/api/counselor/enquiries/${enquiry.id}`, {
        method: "PUT",
  ...
  const handleEditNotes = (enquiry: Enquiry) => {
    setSelectedEnquiry(enquiry);
  ...
  const confirmEditNotes = async (enquiryId: number, note: string, remark: string) => {
    ...
    try {
      const response = await fetch(`/api/counselor/enquiries/${enquiryId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
