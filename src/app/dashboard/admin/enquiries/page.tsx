...
import {
  ...
  type Enquiry,
  type EnquiryStatus,
} from "@/components/dashboard/enquiries";
import { Search, RefreshCw } from "lucide-react";
import { toast } from "sonner";

type StatusTab = EnquiryStatus | "ALL" | "ASSIGNED";
...
export default function EnquiriesPage() {
  ...
  const fetchCounts = React.useCallback(async () => {
    try {
      const response = await fetch("/api/admin/enquiries?counts=true");
  ...
  const handleAssignCounselor = (enquiry: Enquiry) => {
    setSelectedEnquiry(enquiry);
    setAssignCounselorOpen(true);
  };

  const handleAddAgent = (enquiry: Enquiry) => {
    setSelectedEnquiry(enquiry);
  ...
  const confirmAssignCounselor = async (enquiryId: number, counselorId: string) => {
    ...
    try {
      const response = await fetch(`/api/admin/enquiries/${enquiryId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ assignedToId: counselorId }),
      });

      const data = await response.json();
      if (!data.success) {
        toast.error(data.error || "Failed to assign counselor");
        return;
      }

      toast.success("Counselor assigned successfully");
      setAssignCounselorOpen(false);
      fetchEnquiries();
      fetchCounts();
    } catch {
  ...
  const confirmAddAgent = async (enquiryId: number, agentId: string) => {
    setActionLoading(true);
    try {
      const response = await fetch(`/api/admin/enquiries/${enquiryId}`, {
        method: "PUT",
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Manage all course enquiries and leads
          </p>
        </div>
      </div>
      ...
      <AssignCounselorDialog
        enquiry={selectedEnquiry}
        open={assignCounselorOpen}
        onOpenChange={setAssignCounselorOpen}
        onConfirm={confirmAssignCounselor}
