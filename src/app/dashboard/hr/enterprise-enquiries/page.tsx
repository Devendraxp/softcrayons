...
import {
  HREnterpriseEnquiryList,
  HREnterpriseEnquiryListHeader,
  HREnterpriseViewDetailsDialog,
  HREnterpriseEditNotesDialog,
  type EnterpriseEnquiry,
  type EnterpriseEnquiryStatus,
} from "@/components/dashboard/hr-enterprise-enquiries";
...
export default function HREnterpriseEnquiriesPage() {
  ...
  const fetchCounts = React.useCallback(async () => {
    try {
      const response = await fetch("/api/hr/enterprise-enquiries?counts=true");
  ...
  const fetchEnquiries = React.useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/hr/enterprise-enquiries?${params.toString()}`);
      const data = await response.json();
  ...
  const handleStatusChange = async (enquiry: EnterpriseEnquiry, status: EnterpriseEnquiryStatus) => {
    try {
      const response = await fetch(`/api/hr/enterprise-enquiries/${enquiry.id}`, {
        method: "PUT",
  ...
  const confirmEditNotes = async (enquiryId: number, note: string, remark: string) => {
    ...
    try {
      const response = await fetch(`/api/hr/enterprise-enquiries/${enquiryId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Manage enterprise enquiries assigned to you
          </p>
        </div>
      </div>
