  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const submitData = {
        ...
        // Note: isPublic, isFeatured default to false (set by API)
        // Note: authorId is set automatically from session (set by API)
        // Note: dateOfPublish is set to today (set by API)
      };

      const response = await fetch("/api/hr/blogs", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        router.push("/dashboard/hr/blogs");
        router.refresh();
      } else {
  ...
  return (
    <div className="w-full max-w-[98%] mx-auto p-4 space-y-4">
      ...
      <div className="bg-muted/50 border rounded-lg p-4 text-sm text-muted-foreground">
        <strong>Note:</strong> Your post will be saved as a draft and reviewed by an administrator before publication.
        The publish status and featured settings will be managed by admins.
      </div>
