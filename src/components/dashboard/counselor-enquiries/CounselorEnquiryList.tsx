...
import { CounselorEnquiryRow, type Enquiry, type EnquiryStatus } from "./CounselorEnquiryRow";
import { Skeleton } from "@/components/ui/skeleton";
import { cn } from "@/lib/utils";

interface CounselorEnquiryListProps {
  enquiries: Enquiry[];
  isLoading?: boolean;
...
export function CounselorEnquiryList({
  enquiries,
  isLoading = false,
  onViewDetails,
  onStatusChange,
  onEditNotes,
  className,
}: CounselorEnquiryListProps) {
  if (isLoading) {
    return (
      <div className={cn("space-y-3", className)}>
        {Array.from({ length: 5 }).map((_, i) => (
          <CounselorEnquiryRowSkeleton key={i} />
        ))}
      </div>
    );
  }
  ...
  if (enquiries.length === 0) {
    return (
      <div className={cn("py-12 text-center", className)}>
        <div className="text-muted-foreground">No enquiries assigned to you</div>
      </div>
    );
  }
...
function CounselorEnquiryRowSkeleton() {
  return (
...
interface CounselorEnquiryListHeaderProps {
  className?: string;
}

export function CounselorEnquiryListHeader({ className }: CounselorEnquiryListHeaderProps) {
  return (
    <div
      className={cn(
